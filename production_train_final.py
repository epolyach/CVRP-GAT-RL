...
Fixed version with corrected baseline initialization
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('production_training_fixed.log'),
...
class SimpleRolloutBaseline:
    """Simplified rollout baseline that handles batching correctly"""
    ...
    def epoch_callback(self, model, epoch):
        """Update baseline if model improved"""
...
def train_epoch(model, train_loader, optimizer, baseline, epoch, config, writer):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # Calculate reward (negative cost for minimization) - use only x,y coordinates
...
def validate(model, valid_loader, config):
    ...
    with torch.no_grad():
        for batch in valid_loader:
            ...
            # Calculate cost - use only x,y coordinates
...
def main():
    ...
    # Initialize baseline (simplified version)
    baseline = SimpleRolloutBaseline(model, CONFIG)
    
    # Initialize tensorboard
    writer = SummaryWriter(f'runs/production_fixed_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}')
    
    # Create checkpoint directory
    checkpoint_dir = f"checkpoints/production_fixed_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
